...
module.exports = function (app, options, defaultData) {
	...
	function parseStreams() {
		...
		if (streams.constructor.name !== 'Array') {
			throw new Error('streamFactories should be Array');
		}
		if (streams.length === 0) {
			throw new Error('No streamFactory was specified');
		}
	...
	app.logger = function (component, extraData) {

		let opts = _.assign({}, defaultData, {component}, extraData);

		return app._log.child(opts);
	};
};
